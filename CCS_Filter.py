...
def gen_test_strings(barcode, barcode_rc):
    ...
    #First: exact matches
    #exclude
    test_end_in_3pBarcodeRC = "".join(rd.choices(nucleotides, k=100-len(barcode))) + barcode_rc
    test_strings.append(test_end_in_3pBarcodeRC)
    #keep
    test_beg_3pBarcodeRC = barcode_rc + "".join(rd.choices(nucleotides, k=100-len(barcode)))
    test_strings.append(test_beg_3pBarcodeRC)
    #keep
    test_5pBarcodeRandom3pBarcode = barcodes_list[0] + "".join(rd.choices(nucleotides, k=100)) + barcode
    test_strings.append(test_5pBarcodeRandom3pBarcode)
    #exclude
    test_3pBarcodeThenRandom= barcode + "".join(rd.choices(nucleotides, k=(100-len(barcode))))
    test_strings.append(test_3pBarcodeThenRandom)
    #exclude
    test_random="".join(rd.choices(nucleotides, k=(100)))
    test_strings.append(test_random)
    #keep
    test_endIn3pBarcode= "".join(rd.choices(nucleotides, k=100-len(barcode))) + barcode
    test_strings.append(test_endIn3pBarcode)


    #Next: mutated matches
    bar2_list = list(barcode)
    rand_subs_list = rd.choices(np.arange(len(barcode)), k=int(len(barcode)/6))
    ...
    for i in np.arange(int(len(rand_nuc_list)/3)):
        del bar2_list[i]
    delet_bar2_str = "".join(delet_bar2_list)
    delet_bar2_str_rc = gen_rev_complement(delet_bar2_str)

    #exclude
    test_end3RC_sub = "".join(rd.choices(nucleotides, k=100-len(barcode))) + sub_bar2_str_rc
    test_end3RC_insert = "".join(rd.choices(nucleotides, k=100-len(barcode))) + insert_bar2_str_rc
    test_end3RC_del = "".join(rd.choices(nucleotides, k=100-len(barcode))) + delet_bar2_str_rc
    test_strings.append(test_end3RC_sub)
    test_strings.append(test_end3RC_insert)
    test_strings.append(test_end3RC_del)
    #keep
    test_beg3RC_sub = sub_bar2_str_rc + "".join(rd.choices(nucleotides, k=100-len(barcode)))
    test_beg3RC_insert = insert_bar2_str_rc + "".join(rd.choices(nucleotides, k=100-len(barcode)))
    test_beg3RC_del = delet_bar2_str_rc + "".join(rd.choices(nucleotides, k=100-len(delet_bar2_str_rc)))
    test_strings.append(test_beg3RC_sub)
    test_strings.append(test_beg3RC_insert)
    test_strings.append(test_beg3RC_del)

    #keep
    test_5pRand3p_sub = barcodes_list[0] + "".join(rd.choices(nucleotides, k=100)) + sub_bar2_str
    test_5pRand3p_insert = barcodes_list[0] + "".join(rd.choices(nucleotides, k=100)) + insert_bar2_str
    test_5pRand3p_del = barcodes_list[0] + "".join(rd.choices(nucleotides, k=100)) + delet_bar2_str
    test_strings.append(test_5pRand3p_sub)
    test_strings.append(test_5pRand3p_insert)
    test_strings.append(test_5pRand3p_del)

    #exclude
    test_beg3p_sub = sub_bar2_str + "".join(rd.choices(nucleotides, k=100))
    test_beg3p_insert = insert_bar2_str + "".join(rd.choices(nucleotides, k=(100)))
    test_beg3p_del = delet_bar2_str + "".join(rd.choices(nucleotides, k=(100)))
    test_strings.append(test_beg3p_sub)
    test_strings.append(test_beg3p_insert)
    test_strings.append(test_beg3p_del)

    #keep
    test_end3p_sub = "".join(rd.choices(nucleotides, k=100-len(barcode))) + sub_bar2_str
    test_end3p_insert = "".join(rd.choices(nucleotides, k=100-len(barcode))) + insert_bar2_str
    test_end3p_del = "".join(rd.choices(nucleotides, k=100-len(delet_bar2_str))) + delet_bar2_str
...
#            print('iy is', iy)
            ...
            F[i][j] = max(m,ix,iy)
            print_str = "F["+str(i)+"]"+"[" +str(j)+ "] is: "
#            print(print_str,F[i][j])
    #now matrix is filled
    score = max(F[num_rows-1])
    return score

#test function
test_seq = 'ACGTACAGATAGAT'
test_barcode = 'AAGTACA'
test_seq2 = 'ACAGT'
test_barcode2 = 'ACGT'
score_2 = score_alignment(1,test_seq2,test_barcode2,score_dict)
score_sig_2 = calc_score_sig(score_2)
print(score_sig_2)
...
#    print()
    ...
    if barcode_beg_score_sig >0.9:
#        print("beg: barcode in beg. Returning False.")
#        print()
        return False            
    elif  barcodeRC_beg_score_sig>0.9:
#        print('beg: barcodeRC in beg. Returning True')
#        print()
        return True
    #end                        
    elif barcode_end_score_sig > 0.9:
